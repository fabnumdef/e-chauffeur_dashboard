<template>
  <main>
    ...
    <div class="box">
      <vue-calendar
        :events="events"
        :opening-hours="driver.workingHours"
    ...
    <form
      class="box"
      @submit.prevent="updateWorkingHours(driver.workingHours)"
    >
      <ec-field
...
export default {
  ...
  methods: {
    ...
    async updateWorkingHours(oh) {
      this.driver.workingHours = oh;
      const { data } = await this.$api
        .drivers(this.campus.id, 'id,email,workingHours')
        .patchDriver(this.driver.id, this.driver);
      Object.assign(this.driver, data);
    },
